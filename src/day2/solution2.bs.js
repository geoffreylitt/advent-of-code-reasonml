...
function run(input) {
  var func = Tablecloth.$$Array.get;
  var arg = interpret(input, 0);
  return getExn((function (param) {
                  return Curry._2(func, param, arg);
                })(0));
}
...
function runWithInputs(noun, verb) {
  var program = parse(input);
  Tablecloth.$$Array.set(1, noun, program);
  Tablecloth.$$Array.set(2, verb, program);
  return run(program);
}

function range(max) {
  return Tablecloth.List.initialize(max, (function (n) {
                return n + 1 | 0;
              }));
}

function findSolution(_noun, _verb) {
  while(true) {
    var verb = _verb;
    var noun = _noun;
    var result = runWithInputs(noun, verb);
    if (result === 19690720) {
      return /* tuple */[
              noun,
              verb
...
exports.runWithInputs = runWithInputs;
exports.range = range;
